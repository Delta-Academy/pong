...
def play_pong(
    ...
    Args:
        ...
        n_games: how many games to play against your opponent
...
class PongBall:
    ...
    def update(self, paddles: Tuple["PongPaddle", "PongPaddle"]):
        ...
        if new_x <= 0 or new_x >= COURT_WIDTH:
            ...
            if self.has_hit_paddle(paddle_1 if new_x <= 0 else paddle_2, y_at_paddle):
                self.bounce_off_paddle(paddle_1 if new_x <= 0 else paddle_2, y_at_paddle)
        ...
        assert 0 < self.y < COURT_HEIGHT, "Internal error in game - ignore this and rerun!"
        assert (
    ...
    def has_hit_paddle(self, paddle: "PongPaddle", ball_y_at_paddle: float) -> bool:
        ...
        return (paddle.bottom - self.radius) <= ball_y_at_paddle <= (paddle.top + self.radius)
    ...
    def bounce_off_paddle(self, paddle: "PongPaddle", ball_y_at_paddle: float) -> None:
        ...
        self.y = ball_y_at_paddle + (1 - proportion_time_pre_bounce) * self.speed * math.cos(
            self.direction_radians
        )
...
class PongEnv:
    ...
    def __init__(
        ...
        opponent_choose_move: Callable[[Arg(np.ndarray, "state")], int] = robot_choose_move,
    ...
    def step(
        ...
        assert player_action in VALID_ACTIONS, f"Invalid action, action must be in {VALID_ACTIONS}"
        ...
        if self.render and self.timesteps % 10 == 0:
            draw_game(env=self, screen=self.screen)
    ...
    def __repr__(self):
        ...
        paddle_1_y = math.floor((self.paddle_1.centre_y / COURT_HEIGHT) * self.viz_dims[1])
        paddle_2_y = math.floor((self.paddle_2.centre_y / COURT_HEIGHT) * self.viz_dims[1])
...
def init_pygame() -> pygame.surface.Surface:
    ...
    screen = pygame.display.set_mode(VISUALS_DIMS)
    pygame.init()
...
def draw_game(env: PongEnv, screen: pygame.Surface) -> None:
    ...
    pygame.gfxdraw.box(
        screen,
        pygame.Rect(
            ORIGIN[0] + round((COURT_WIDTH)),
            ORIGIN[1] + round(env.paddle_2.bottom),
            paddle_width,
    ...
    pygame.gfxdraw.aacircle(
        screen,
        ORIGIN[0] + round((EXTRA_WIDTH + env.ball.x)),
        ORIGIN[1] + round(env.ball.y),
    ...
    # Draw the score
    img = font.render(f"{env.player_score}", True, BLUE_COLOR, LIGHT_GRAY_COLOR)
    rect = img.get_rect()
    rect.center = (
    ...
    img = font.render("Player", True, BLUE_COLOR)
    rect = img.get_rect()
    rect.center = (ORIGIN[0], ORIGIN[1] + COURT_HEIGHT + img.get_height())
    screen.blit(img, rect)

    img = font.render(f"{env.opponent_score}", True, RED_COLOR, LIGHT_GRAY_COLOR)
    rect = img.get_rect()
    rect.center = (
        ORIGIN[0] + pixel_game_width + font.get_height() // 2,
        ORIGIN[1] + font.get_height() // 2,
    )
    ...
    img = font.render("Opponent", True, RED_COLOR)
    rect = img.get_rect()
    rect.center = (ORIGIN[0] + COURT_WIDTH, ORIGIN[1] + COURT_HEIGHT + img.get_height())
    screen.blit(img, rect)

    img = font.render(f"Ball speed: {round(env.ball.speed, 2)}", True, BLACK_COLOR, None)
    rect = img.get_rect()
    rect.center = (
        ORIGIN[0] + pixel_game_width // 2,
        ORIGIN[1] + pixel_game_height + font.get_height() // 2,
    )
